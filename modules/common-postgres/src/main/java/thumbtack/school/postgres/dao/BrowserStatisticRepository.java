...
@Repository
public interface BrowserStatisticRepository extends CommonRepository<BrowserStatistic> {
    @Override
    @Query("select new thumbtack.school.postgres.dto.StatisticDto(statistic.name, SUM(statistic.count)) " +
            "from BrowserStatistic as statistic where statistic.createdAt between ?1 and ?2 group by statistic.name")
